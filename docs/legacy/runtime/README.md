...
> **已归档（v2.8.0）**：实验性原型（stage 3-8 未实现）整体移入 `docs/legacy/runtime/`，不在默认流程，运行时不加载；保留供复核"必停点由代码强制"的设计（LLM 只能申请、人工经 CLI 登记）。归档后如需离线试跑，在本目录内执行（conftest/pyproject 均随目录移动）。
