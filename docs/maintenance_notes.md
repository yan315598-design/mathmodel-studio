...
| 设计理由/历史机制说明 | 历史归档（v3.3.0 起移出公开仓库，维护者本地留存） | 运行路径不承载 |
...
|---|---|---|
| architecture.md（仓库级架构说明，原公开归档件） | 已归档（v2.8.0），v3.3.0 起随历史归档目录整体移出公开仓库（维护者本地留存，设计理由以本地归档为权威） | 维护者本地归档；运行文档不需要 |
| `references/workspace_protocol.md` §13「知识架构设计理由」 | 已迁出运行路径（2026-09-19） | 同上；原位仅留一行指针 |
| distilled_structures.md（章节结构提示，原归档件） | 已废止并归档，v3.3.0 起移出公开仓库；残值保留 | `competitions/cumcm/phrase_bank.md` §13「章节模板卡」+ `references/stage_08_writing.md` 写作顺序 |
| distilled_formats.md（格式/编号示例，原归档件） | 已废止并归档，v3.3.0 起移出公开仓库；残值保留 | `competitions/cumcm/phrase_bank.md` §13 + `references/cn_presentation_spec.md` §2.7/§5 |
| distilled_phrases.md（句式模板，原归档件） | 已废止并归档，v3.3.0 起移出公开仓库 | `competitions/cumcm/phrase_bank.md` 各章 |
| runtime/ 薄执行器原型（原归档件） | 已归档（v2.8.0），v3.3.0 起移出公开仓库，不在默认流程 | `SKILL.md` 必停点节"宿主强制未启用时的口径" |
...
| `scripts/skill_paths.py` 之前被描述为"两份独立安装并存" | 已纠偏（2026-09-19） | 按调用路径解析并打印当前根；链接入口与真实路径的边界见 `scripts/skill_paths.py`；入口路径约定见 `SKILL.md` §2 |
| 版本历史叙述（SKILL.md / README 内的逐项清单） | 只留短标识 + 指针 | `CHANGELOG.md`（唯一权威源；未核验项如 D2/C7 不得在任何文档宣称完成） |

## 3. 加载落点

任务/场景的章节级加载套餐在 `references/workflow_entry_details.md` §2 维护；阶段索引在 `SKILL.md` §4，竞赛加件在该细则 §10。本文件不复制套餐表。

## 4. 维护期规则

1. **判层必摘**（`references/README.md` §二）：判"旧层/弃用/已归档"时同一改必须摘加载表、grep 清零全库正面引用（`CHANGELOG.md` 历史叙述豁免）、残值先迁移再移出公开仓库（维护者本地归档；原历史归档目录已于 v3.3.0 整体移出）。
