...
当前版本 v3.3.0 的改动集中在出图：多个子图可以拼成一张正式图，画布宽度按论文版心用毫米设定，图例和色条可以跨面板共用。仓库里附了六套合成示例图（A–F，对应调度、通信、图像重构、时空场、信号、空间评价六类场景），可以整套运行，也可以只取其中几个面板。已有的模板和定稿风格都不受影响。图生成后会过一遍自动检查，但检查通过不等于图就合格，用不用还得你自己看过再定。整个项目的结构见 [ARCHITECTURE.md](ARCHITECTURE.md)。
