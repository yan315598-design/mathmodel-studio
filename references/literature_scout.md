...
| Stage 5 翻车点验证 | 某子问验证翻车（不必等到固定返工轮数），怀疑方法本身不适用 | 按profile定向检索（方法名 + 失效场景），寻找失效证据、适用条件、实现修正或其他可行方向；每次检索登记 `decision_log.stages.5.literature_searches` |
...
1. 影响当前选型的主要风险已经有可追溯的依据或实验方向；没有足够依据的部分已经记录为未验证风险；
