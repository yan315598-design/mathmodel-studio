...
- 多类型附件先按 `references/multimodal_assets.md` 检查元数据和读取范围，再完成本阶段的数据接口核验。
...
解析题面后登记每问需要回答的问题、数据来源与验证证据。先按 `references/experiment_cycle.md` 做探索和基线，再确定图型与面板；Stage 2 的图表方案是可修订假设，不能预设结果或提前锁定图数。
...
4. 正式结果验证通过后，在 Stage 5 D.1 确认图型与规格并冻结；此前根据实验修订计划。冻结后变更在 `真源.md` 修订记录登记时间、原因和影响。
