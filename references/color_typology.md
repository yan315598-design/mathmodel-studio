...
> **管辖边界（v2.8.0 起）**：色值（含 NEUTRALS 与示意图色族）唯一权威源 = 本文件 + `palettes.py`；字阶、版式、间距令牌唯一权威源 = `references/design_tokens.md`（不归本文件管）。
...
### 配色语义表（v3.0.0 新增，图叙事语义绑定层）

图叙事要素（判据线/基线/主模型/对照/达标/风险）到颜色与线型的绑定以本表为准，跨色板通用；上方"语义绑定示例"是 `academic_blue` 一套色板的落地写法。图叙事纪律本身（何时必须画判据线、对照式构图、量化标签等）唯一权威源是 `references/figure_skill_bridge.md` 图叙事章，本表只管色与线型。

| 图叙事要素 | 绑定 | 说明 |
|---|---|---|
| 判据线（阈值/判据/理论值） | **红 · 虚线**，图例注明判据值 | 判据必须画进图内（判据线纪律） |
| 基线/参考线 | **灰黑 · 虚线** | 取 NEUTRALS（`secondary`/`arrow`）或色板灰，不参与数据叙事 |
| 主模型/主方案 | **主色**（所选色板第 1 色） | 全文同色（硬规则 1） |
| 对照/基线模型 | **对色或灰** | 色板第 2 色或中性灰，与主模型永不混色 |
| 达标/盈利区 | **绿** | 只用于正面语义（硬规则 10） |
| 风险/亏损区 | **红** | 只用于负面语义（硬规则 10） |
| 网格辅助线 | 淡灰虚线 `alpha ≤ 0.3` | 即硬规则 8，无语义 |

**与 §5 十二条硬规则的关系**：硬规则是约束层（怎么用色——同义同色/禁彩虹/灰度校验/红绿定向等），本表是**语义绑定层**（哪个图叙事要素绑哪个色与线型），不放宽任何一条硬规则；两处重叠的口径（网格 alpha、红绿定向）以硬规则为准。一处明确分工：**线型判据要素（判据线/阈值线/参考线）自 v3.0.0 起按本表绑定**（判据红虚线、基线灰黑虚线），硬规则 10 的"橙=强调"用于数据点/区域级强调，不扩展到判据线。**连续/发散色标（colormap）的端色不作语义定向**——RdBu_r 高值端红表达"大/达标"时是数值映射不是风险语义（复审 P2-7）。
