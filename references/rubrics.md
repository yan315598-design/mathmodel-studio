...
| 维度 | 满分行为 |
|------|---------|
| 1. 候选比较 | 根据主要不确定性选择有意义的比较对象，说明比较如何改变判断；没有足以改变判断的对象时披露限制和未验证风险 |
| 2. 选型理由 | 每个候选有 (a) 适配理由 (b) 不选的原因 |
| 3. 名称与机制 | 名称对应公式与实现，新增模块有必要性与接口，不要求修饰词 |
| 4. 求解可行性 | 关键实际机制、接口与资源有验证证据；未覆盖部分明确，不能以库存在或toy成功代替正式实现 |
| 5. 选型依据（保留评分键5_literature_support） | 关键主张和否决理由有可核验依据：相关原文、同构案例、题面机理推导或本题实验均可；区分假设、实现与已证效果。外源机制应核验出处和适用条件，不强制篇数或主模型外源配额 |
