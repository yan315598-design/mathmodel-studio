...
### 2.5 组合预测
- **核心思想**: 多个预测器通过加权、堆叠、残差校正或其他方式形成联合输出
- **常见形态**: "基于误差的加权预测"、"预测器与残差校正模型串联"
- **使用边界**: 组合预测只是预测类的一种候选结构；是否采用组合由本题的目标、数据、误差结构和实验结果决定
...
- **国赛经验**: AHP 可独立使用，也可与 TOPSIS、熵权等组合；是否组合取决于指标结构、权重来源和敏感性结果
...
模型、表示、数据处理、求解器、校正项和验证模块可以采用单一、串联、嵌套、并行或其他组织方式。选择组织方式时记录：

- 每个模块接收什么输入、产生什么输出；
- 它解决哪个具体问题，或检验哪个可被否定的假设；
- 模块之间是否存在重复计量、信息泄漏或误差传递；
- 整体方案相对于简化方案增加了什么收益、代价或适用边界。

没有证据支持增加模块时保留更简单的结构；有证据支持多个模块承担不同作用时可以采用组合结构。两种情况都以本题实验、机理分析或文献证据说明。

---

## stage 3 选型 checklist

入围候选应记录；探索中的候选可先简记，只有进入正式比较或移交时才补齐关键字段:
- [ ] 来自哪个族 (1-10, 9A)
- [ ] 适配理由（覆盖关键选型主张与最高风险边界，行数不限）
- [ ] Python 实现路径 (库/自实现)
- [ ] 时间复杂度估算
- [ ] 名称与实际改动一致，不伪造“改进”
- [ ] 已考虑的其他解释、实现或路线及其取舍；若没有足以改变判断的比较对象，记录限制和未验证风险

候选数量和结构按证据决定；记录哪些比较改变了判断，以及哪些风险仍未验证（championship 同样适用）。
...
**Stage 3 选型输出的每个 Qi 条目应含"首选 + 首选失效条件"**；存在可信备用时附"备用 + 触发条件"，不存在时写明限制证据，不强行凑三件套。失效条件触发后重新评估备用或回到探索，不把原先备用视为自动正确答案。写入 `decision_log.stages.3.selected_per_subproblem` 的每个 Qi 条目。
