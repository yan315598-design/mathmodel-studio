...
- 每个 panel 是否提供独立证据。
- 文字是否可读, 没有出界和重叠。
- 坐标、单位、图例是否齐全；图注（图名+结论）在 caption 而非图内。
- 同一模型或方法在全文是否保持同一颜色。
- 是否有可编辑 SVG/PDF 版本。
...
### 3.6 图内标题与标注纪律（对齐优秀论文惯例, 1.4.1 新增）

对齐优秀论文/期刊惯例, 数据图（Type 3）**图内不放图名与说明文字, 图名和结论写进 caption**（由 LaTeX/Word 排版"图 X xxx"题注）:

- **不放**: 图题（matplotlib 的 suptitle / 单面板 set_title）、结论句、大段说明文字、数据来源注、装饰性边框/背景色块。
- **保留**: 轴标签（含单位）、刻度与刻度标签、图例、多面板标号 (a)(b)(c)、必要参考线/关键点标记（全文 ≤3 处且正文有对应论证句）。
- **示意图豁免**: 流程图/技术路线图/框架图的标题条是版式组成部分, 不受本条约束（走 `--allow-box-labels` / `--allow-infigure-title` 通道）。
- **panel 短标签**: 多面板图各 panel 允许 ≤6 个中文字符当量的轴含义短标签（如 "ROC"、"PR"、"残差"）, 超出部分移入 caption。
- **门禁**: `figure_lint.py` R8（非空 suptitle 或单面板 set_title 报错; 多面板标题超宽警告）; 示意图等特殊版式用逃生门 `--allow-infigure-title` 跳过。
