...
> v7.4.0 新增。为华为杯等 72-96h 赛制设计，适用于全部竞赛分支。
...
3. 跑 `scripts/freeze_numbers.py check` 与 `scripts/run_manifest.py verify`（v7.5.0 配套脚本，见 §9/§10）→ 报告 stale 项；工作区里没有这两个脚本时跳过本步，不报错不阻塞。
...
> v7.5.0 新增。`scripts/freeze_numbers.py` 为 v7.5.0 配套脚本（可能尚未落盘，落盘前本节按约定口径执行）。**frozen_numbers 是 §5"h60 结果数字冻结"的机器执行层**：§5 定时点（60h 起只写不改模型），本节定登记与校验的落地形式， 两者一条链， 不构成第二套口径。
...
> v7.5.0 新增。`scripts/run_manifest.py` 为 v7.5.0 配套脚本（可能尚未落盘）。解决"哪次运行产出了这份结果、结果还能不能信"的追溯问题。
