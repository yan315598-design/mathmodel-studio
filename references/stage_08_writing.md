...
先按 `references/modeling_evidence_protocol.md` 核对答题义务与原始结果；trace_claims 的链条通过不等于结论语义正确。重点检查收敛、最优性、分母、区间和数据划分；允许无改进，未完成内容不可写成全部解决。
...
- 图名与图的结论写在图下方 caption（“图 X：…”），图内不烘焙标题（suptitle / 单面板 set_title）；多面板 panel 允许 ≤6 个中文字符当量的轴含义短标签（如 ROC/PR/残差），超出移入 caption。示意图豁免：figqa 走 `--allow-box-labels`（该参数只豁免示意图盒内文字，不关闭 figure_lint R8 门禁）；若示意图仍被 figure_lint R8 检出（如 matplotlib 直出示意图），用 `--allow-infigure-title` 逃生门跳过。
- 正文引用图为“如图 X 所示”，不指向图内文字。
...
每条带支撑且被某个结论需要 (winning_patterns.md 假设支撑相关节)，不设条数区间。
...
覆盖全部必要符号, 全单位 (anti_pattern B5)，不设行数下限。
...
```
5.1 问题一: <与实际实现一致的真实模型名>

5.1.1 模型建立

本节针对问题一建立 <名称只反映实际公式与实现, 见 stage_03 命名规则> 模型。
...
针对 (5.1)-(5.3), 我们用 <实际算法/求解器> 求解（名称与代码实现一致；模型与求解技术分开表述）, 具体步骤如下:
**Step 1**: ...
**Step 2**: ...
...
**强制 checklist** (winning_patterns.md 摘要证据链/模型约束驱动/子问依赖链/结果可追溯各节):
- [ ] 模型名称对应公式与正式实现，不强制修饰词
- [ ] 公式编号规范、符号与 §4 一致
- [ ] 图表数量由论证需要决定，每张绑定主张与上游数据
- [ ] 物理意义段：数值 → 现实含义
- [ ] (Q3) 题面要求复用时显式引用 Q1/Q2 结果；题面指定固定输入时保持独立并说明依据
...
为评估模型对参数变化的敏感程度, 本文采用 <stage 6 实际方法与依据> 
对 <参数> 进行扰动（方法与幅度按 stage 6 登记的依据填写，不套用默认档位）。
...
(1) **<缺点 1>**: 当前 <现象>，会使 <结果> 向 <方向> 偏。若改用 <替代方法>, 
    预期 <有依据才量化的改进>, 但需 <代价>。
(2) ...
(3) ...
...
类型有混合 (中文期刊/英文期刊/教材或工具书)，配比按题目领域需要，不设固定数量配比。
...
对模型按 stage 6 实际登记的方法与范围做了灵敏度分析, 在 <实际测试范围> 内, 
<结论是否翻转/偏差范围>, 说明 <成立条件或失效边界>。
...
- 定量结果可追踪 (anti_pattern A1)；数量按题问需要
- 摘要结构与所加载竞赛模板一致 (anti_pattern A2)
...
| 4. 语言质量 | 语言自然准确、无危险句式；phrase_bank 仅作参考，命中率不作评分依据 |
...
6. **结论-结果核验通过**: `python scripts/claim_consistency_check.py --draft paper_workspace/ --results results/ --strict` exit 0（正文强结论与结果文件状态无直接矛盾；warn/info 人工核对后在真源.md 登记处理结果）
7. **证据账本完整**: `state/evidence_ledger.json` 中摘要实际覆盖的每个 Qi 行都必须有非空 `abstract_claim`；不能只填当前问。摘要数字抽查须能在结果文件中找到同值（claim check 规则 5 辅助；抽查结果登记真源.md）
8. **真源对账**: 写作前抽查真源.md 结果登记表的数值与结果文件一致（可用 `scripts/freeze_numbers.py check` 辅助）；发现漂移以结果文件为准并在修订记录登记，不得照抄漂移值
9. L1 全维 ≥7
10. **L2 跨阶段回检**: 论文内容与 decision_log 0-7 一致, 无新增内容偏离
