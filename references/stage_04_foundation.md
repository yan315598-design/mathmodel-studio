...
- 假设清单 (数量按风险需要,每条带支撑且被某个结论需要) → 论文 §3
- 符号说明表 (含单位、类型) → 论文 §4
...
数量不设区间：每条假设都应有结论依赖它；删掉不影响任何结论的假设不保留，结论需要而没写的假设必须补。
...
- B1 假设无支撑 → Step 2 强制
- B2/B3 假设过多/过少 → 按结论需要增减，每条注明哪个结论依赖它
- B4 符号重复 → Step 3 自动检
- B5 无单位 → Step 3 自动检
...
1. 假设每条有支撑，且每条都被某个结论需要
2. 符号表覆盖全部必要符号, 全有单位与类型
3. 术语定义完整 (若适用)
4. 一致性预检通过
5. L1 全维 ≥7
...
stage 8 写论文 §3 §4 时,直接复用本文产出 (winning_patterns.md 假设支撑相关节)。
