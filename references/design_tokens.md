...
适用范围：本文件卡片、网格、节点数量是示意流程图的设计建议，不限制真实网络/物理几何/影像。科学与交付硬约束以 `references/figure_quality_layers.md` 为准。
...
- **字号扁平纪律**（全图不超过 4 档）：图标题 16、标题条 12、卡标题 10.5、
  明细/注释 9。
- 流程卡片可承载标题和明细；真实网络/几何对象可直接标注。按实际遮挡检查文字，画布高按内容和入文尺寸确定。
...
**① 网格与对齐（先排栅格，再写图元）**
- 流程示意图可用 4px 网格对齐（`figkit.snap4()` / `drawio_builder.snap4()`）；真实几何坐标和数据点保持原值。
...
**⑤ 密度与删除纪律**
- 节点较多时考虑总览加细节；是否拆图由真实关系与最终版心可读性决定，不设通用节点上限。
...
- ❌ 坐标不吸 4px 网格（先 `snap4()`）；
- 节点密集且最终尺寸不可读时拆为总览与细节；不按固定节点数裁掉必交关系。
