# -*- coding: utf-8 -*-
"""竖版五带技术路线图（示意图模板包 · roadmap, v7.9.1 语义槽位重写版）。

五个横向色带层自上而下排布: 问题提出 → 数据处理 → 模型构建 → 求解验证 →
结论应用。v7.9.1 起从"一行文字通用卡"升级为**语义槽位版式**（追平
...
每条 1-3 个节点; 节点也兼容纯字符串（退化为单行卡）。向后兼容 v7.8.0 的
...
版本:
    v7.9.1: 语义槽位重写——两段式富文本卡/圈号徽章/竖排阶段目标/结论脚注,
        字重回层级制（标题条与徽章加粗, 卡内正文常规）, 连线降重。
    v7.8.0: 迁移示意图色族体系; v7.7.0: 迁移 figkit + 三格式导出。
...
# 示例数据 v7.9.1: (层名, 阶段目标, [(标题, 明细), ...]); 每层 1-3 个节点
...
def _normalize_bands(bands) -> list[tuple[str, str, list[tuple[str, str]]]]:
    """入参归一化: 兼容 v7.8.0 二元组 (层名, [文字...]) 与 v7.9.1 三元组
...
def plot_roadmap(bands, out_stem: str | None = None,
    ...
    Args:
        bands: 恰好 5 条; 推荐 v7.9.1 三元组 (层名, 阶段目标, [(标题, 明细)...]),
