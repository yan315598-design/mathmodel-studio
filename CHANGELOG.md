...
## [3.4.1] 去除模型路线与数量锚定（2026-09-22）

- 将候选生成、实验比较、文献停止条件和选型评分统一为问题驱动，不预设基线/替代路线、候选数量或组合形式。
- 明确单一方法、模块化方法和组合结构均可按题目证据采用；保留机制、实现、公平比较和未验证风险记录。
- 修正模型目录、华为杯/CUMCM 参考文档及首页说明中的固定路线暗示；不改变 Stage、schema、插件接口或门禁结构。

本变更必须同改的文件：`SKILL.md`、`README.md`、两份 plugin.json、`PACKAGE_CONTENTS.md`、`CHANGELOG.md`、Stage 3/实验/证据/文献/评分参考及相关竞赛文档。
